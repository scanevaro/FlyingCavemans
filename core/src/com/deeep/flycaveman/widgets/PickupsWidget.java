...
public class PickupsWidget extends Actor {
    ...
    public PickupsWidget() {
        ...
        stamina.setPosition(-stamina.getWidth() - 20, Core.VIRTUAL_HEIGHT / 2 - stamina.getHeight());
        ...
        infinite.setPosition(-infinite.getWidth() - 45, Core.VIRTUAL_HEIGHT / 2 - infinite.getHeight() / 2);
        one = new Image(Assets.one);
        one.setSize(one.getPrefWidth(), one.getPrefHeight());
        one.setPosition(-one.getWidth() - percentage.getWidth(), Core.VIRTUAL_HEIGHT / 2 - one.getHeight() / 2);
        five = new Image(Assets.five);
        five.setSize(five.getPrefWidth(), five.getPrefHeight());
        five.setPosition(-five.getWidth() - percentage.getWidth(), Core.VIRTUAL_HEIGHT / 2 - five.getHeight() / 2);
        plus = new Image(Assets.plus);
        plus.setSize(plus.getPrefWidth(), plus.getPrefHeight());
        plus.setPosition(-plus.getWidth() - one.getWidth() - percentage.getWidth(), Core.VIRTUAL_HEIGHT / 2 - plus.getHeight() / 2);
        minus = new Image(Assets.minus);
        minus.setSize(minus.getPrefWidth(), minus.getPrefHeight());
        minus.setPosition(-minus.getWidth() - five.getWidth() - percentage.getWidth(), Core.VIRTUAL_HEIGHT / 2 - minus.getHeight() / 2);
    }
        ...
        if (type == PowerUp.Type.VODKA) {
            ...
            plus.addAction(new SequenceAction(
                    ...
                    Actions.moveTo(-plus.getWidth() - five.getWidth() - percentage.getWidth(),
                            Core.VIRTUAL_HEIGHT / 2 - plus.getHeight() / 2, 0.3f)));
            ...
            one.addAction(new SequenceAction(
                    Actions.moveTo(64, Core.VIRTUAL_HEIGHT / 2 - one.getHeight() / 2, 0.3f),
                    Actions.scaleBy(0.25f, 0.25f, 0.2f), Actions.scaleBy(-0.25f, -0.25f, 0.2f),
                    Actions.delay(0.25f),
                    Actions.moveTo(-one.getWidth() - percentage.getWidth(), Core.VIRTUAL_HEIGHT / 2 - one.getHeight() / 2, 0.3f)));
            plus.addAction(new SequenceAction(
                    Actions.moveTo(0, Core.VIRTUAL_HEIGHT / 2 - plus.getHeight() / 2, 0.3f),
                    Actions.scaleBy(0.25f, 0.25f, 0.2f), Actions.scaleBy(-0.25f, -0.25f, 0.2f),
                    Actions.delay(0.35f),
